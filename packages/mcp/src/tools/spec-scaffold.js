...
const SCAFFOLD_KINDS = [...new Set([...ENTITY_KINDS, 'system', ...ENTITY_KINDS_0_20_0])];
...
export const specScaffoldDef = {
  ...
  description:
    ...
    'For real 0.20.0 (.dsds.yaml), pass spec:"0.20.0" — "system" and "entry" are 0.20.0-only regardless of this flag.',
  inputSchema: {
    type: 'object',
    properties: {
      kind: {
        type: 'string',
        enum: SCAFFOLD_KINDS,
        description: 'Entity kind to scaffold. Use "system" for a multi-entity document (legacy) or the system entry kind (0.20.0).',
      },
      spec: {
        type: 'string',
        enum: ['0.15.2', '0.20.0'],
...
export async function specScaffoldHandler({ kind, spec }) {
  const is20Only = kind === 'entry';
  if (spec === '0.20.0' || is20Only) {
    const scaffold20 = SCAFFOLDS_0_20_0[kind];
    if (!scaffold20) {
      return {
        isError: true,
        content: [{ type: 'text', text: `Unknown 0.20.0 kind "${kind}". Valid kinds: ${Object.keys(SCAFFOLDS_0_20_0).join(', ')}` }],
      };
    }
    const lines = [
