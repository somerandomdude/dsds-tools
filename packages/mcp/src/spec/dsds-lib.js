...
/** Parse DSDS YAML, keeping dates as the plain strings the schema expects. */
export function loadYaml20(text) {
  return yaml.load(text, { schema: yaml.JSON_SCHEMA });
}

/** Parse the DSDS YAML file at `absPath`. */
...
const WELL_KNOWN_KINDS_0_20_0 = new Set(['system', 'component', 'token', 'theme', 'entry']);
const NAMESPACED_KIND_PATTERN = /^[a-z0-9]+(-[a-z0-9]+)*(\.[a-z0-9]+(-[a-z0-9]+)*)+$/;
