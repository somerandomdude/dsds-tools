...
/** Render one entry by identifier, mirroring what the CLI does. */
function render(systems, identifier) {
  const entity = systems.flatMap(s => s.entities).find(e => e.identifier === identifier);
  if (!entity) return { missing: true, text: '' };
  return { text: entityToMarkdown(entity, {}) };
}
...
describe('entityToMarkdown', () => {
  it('renders a legacy 0.15.2 entity to markdown', async () => {
    ...
    const result = render(systems, 'button');
  ...
  it('returns isError for an unknown entity', async () => {
    const { systems } = await loadSystems([`${fixturesDir}/button.dsds.yaml`]);
    const result = render(systems, 'nonexistent');
    expect(result.missing).toBe(true);
  });
});

describe('entityToMarkdown — real 0.20.0 (.dsds.yaml)', () => {
...
describe('entityToMarkdown — sections bucketed by tag', () => {
  ...
  // Built in memory rather than loaded: these cases are about how a tag
  // routes a section to a heading, not about loading.
  const renderTagged = (sections) => {
    const doc = entity(sections);
    return entityToMarkdown({ ...doc.entries[0], identifier: 'tagged', __dsds20: true }, {});
  };

  it('renders a guidelines section tagged accessibility under Accessibility', async () => {
    const text = renderTagged([
