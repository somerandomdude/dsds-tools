...
- Current inspected Text Input and Checkbox code does not expose host-level
  `value` / `checked` accessors for live user state, and the selected controls
  do not establish native form association. Verify this on the implementation
  branch before choosing the smallest fix.
