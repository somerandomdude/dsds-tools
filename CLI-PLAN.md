...
> - **Still open:** npm publish + naming (§10.2 — `dsds` squatted; `dsds-cli` and `dsds-tools` verified free; `@dsds` scope unverified; PJ's call), GitHub repo rename to dsds-tools, Stage B core extraction, CI wiring in the docs repo (§10.5), lint plugin install location (Phase 3 doctor finding), search tokenization (Phase 4 demo note).
